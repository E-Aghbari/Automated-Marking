...
def partitionData(training_data: numpy.typing.NDArray, f: int) -> list[numpy.typing.NDArray]:
    ...
    # Split the data into f folds
    for i in range(f):
        # Calculate the start and end index of the testing data for the current slice
        start_index = i * partition_size
        end_index = (i + 1) * partition_size
        data = training_data[start_index:end_index]
...
def evaluateResults(classified_data_list: list[numpy.typing.NDArray], evaluation_func=Task_2.evaluateKNN) \
    ...
    # calculate values for each fold in the data

    # seperate them into lists based on their folds, then calculate these values and add to list

    # finds highest value in the foldNumber column to figure out how many folds there are
    f = numpy.max(classified_data_list[1:, 3].astype(int)) + 1
    for i in range(f):
        # the rows in foldNumber column that match foldNumber i
        idx = classified_data_list[:, 3] == str(i)
        # numpy array with the data for this fold
        fold = classified_data_list[idx]

        # calculating values for the fold
        precision, recall, f_measure, accuracy = evaluation_func(fold)
    ...
    avg_f_measure = sum(f_measure_lst) / len(f_measure_lst)
    avg_accuracy = sum(accuracy_lst) / len(accuracy_lst)
...
def crossEvaluateKNN(training_data: numpy.typing.NDArray, f: int, k: int, measure_func: Callable,
    ...
    folds = prep_func(partition_func(training_data, f),f)

    # put every fold into the knn classifier
    for i in range(f):
        classified_data = knn_func(folds[i][2],folds[i][1], k, measure_func, similarity_flag)
        if i == 0:
            fold_numbers = numpy.zeros(classified_data.shape[0], dtype=int)
        else:
            fold_numbers = numpy.ones(classified_data.shape[0], dtype=int) * (i)
        # add a column of the fold number to the classified data
        added_column = numpy.c_[classified_data, fold_numbers]
        processed = numpy.concatenate((processed, added_column[1:, :]))


    # find out the average precision, recall, accuracy and f-measure
    avg_precision, avg_recall, avg_fMeasure, avg_accuracy = eval_func(processed)
