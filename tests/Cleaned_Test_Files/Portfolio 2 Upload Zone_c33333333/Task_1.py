...
def getMostCommonClass(nearest_neighbours_classes: dict[str, int]) -> str:
    ...
    if not nearest_neighbours_classes:
        return winner

    for class_name in classification_scheme:
        if class_name in nearest_neighbours_classes:
            return class_name
...
def getClassesOfKNearestNeighbours(measures_classes: list[tuple[float, str]], k: int, similarity_flag: bool) -> (
    ...
    for i in range(stop):
        if measures_classes[i][1] in nearest_neighbours_classes:
            nearest_neighbours_classes[measures_classes[i][1]] += 1
...
def kNN(training_data: numpy.typing.NDArray, data_to_classify: numpy.typing.NDArray, k: int, measure_func: Callable,
    ...
    if len(training_data) == 0 or len(data_to_classify) == 0:
        return classified_data

    # remove header row
    data_to_classify = data_to_classify[1:]

    # go through each data point that needs classifying
    for data in data_to_classify:
        image_path = data[0]
        # read and resize
        items = read_func(image_path)

        measure_classes = []
        training_data = training_data[1:]

        # go through training datat to get measures
        for train in training_data:
            train_item = read_func(train[0])
            similarity = measure_func(items, train_item)
            class_label = train[1]
            measure_classes.append((similarity, class_label))

        # sort measure classes
        measure_classes.sort()

        # get nearest neighbour classes
        nearest_neighbour_classes = get_neighbour_classes_func(measure_classes[:k], k, similarity_flag)

        # get predicted class
        predicted_class = most_common_class_func(nearest_neighbour_classes)

        # add classified data to the classified data array
        classified_data = numpy.append(classified_data, [[data[0], data[1], predicted_class]], axis=0)
