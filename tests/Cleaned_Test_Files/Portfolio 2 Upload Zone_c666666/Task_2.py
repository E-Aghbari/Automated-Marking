...
def confusionMatrix(classified_data: numpy.typing.NDArray) -> numpy.typing.NDArray:
    # Extract actual and predicted classes from the classified_data
    actual_classes = classified_data[:, 1]
    predicted_classes = classified_data[:, 2]

    # Get the unique classes in the data
    classes = numpy.unique(numpy.concatenate((actual_classes, predicted_classes)))
...
def computeMacroPrecision(tps: list[int], fps: list[int], fns: list[int], data_size: int) -> float:
    ...
    # Calculate precision for each class and append to precision list
    for index in range(len(tps)):

        # If divisor is going to be 0, skip this iteration
        if tps[index] + fps[index] == 0:
            continue

        precision.append(tps[index] / (tps[index] + fps[index]))
...
def computeMacroRecall(tps: list[int], fps: list[int], fns: list[int], data_size: int) -> float:
    ...
    # Calculate recall for each class and append to recall list
    for index in range(len(tps)):
        ...
        recall.append(tps[index] / (tps[index] + fns[index]))
...
def computeMacroFMeasure(tps: list[int], fps: list[int], fns: list[int], data_size: int) -> float:
    t = 0
    for n in range(0, 5):
        micro_recall = (tps[n]) / (tps[n] + fns[n])
        micro_presion = (tps[n]) / (tps[n] + fps[n])
        micro_f_measure = (2 * micro_recall * micro_presion) / (micro_recall + micro_presion)
        t += micro_f_measure
    f_measure = t / 3
    return f_measure
