...
def preparingDataForCrossValidation(partition_list: list[numpy.typing.NDArray], f: int) \
    ...
    for i in range(0, f):
        ...
        folds.append((i+1, numpy.array(new_train), numpy.array(new_test)))
    return folds
...
def crossEvaluateKNN(training_data: numpy.typing.NDArray, f: int, k: int, measure_func: Callable,
    ...
    for i in range(0, len(folds)):
        ...
        precision, recall, fmeasure, accuracy=Task_2.evaluateKNN(result)
        avg_precision+=precision
        avg_recall+=recall
    ...
    v = [avg_precision/f, avg_recall/f, avg_fMeasure/f, avg_accuracy/f]
