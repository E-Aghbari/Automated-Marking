...
def preparingDataForCrossValidation(partition_list: list[numpy.typing.NDArray], f: int) \
    ...
    for i in range(0, f):
        ...
        folds.append((i, numpy.array(new_test), numpy.array(new_train)))
    return folds
...
def evaluateResults(classified_data_list: list[numpy.typing.NDArray], evaluation_func=Task_2.evaluateKNN) \
    ...
    result = evaluation_func(classified_data_list) #gets tuples of calculations

    avg_precision = result[0] #assigns calculations based off of the order in which they are returned
    avg_recall = result[1]
    avg_f_measure = result[2]
...
def crossEvaluateKNN(training_data: numpy.typing.NDArray, f: int, k: int, measure_func: Callable,
    ...
    splitData = prep_func(partitions, f)
    classified_list = []
    # Have fun with the computations!
    precisions = []
    recalls = []
    ...
    for fold in splitData:  # for each fold
        foldNum = fold[0]

        train = fold[1]
        test = fold[2]

        splitClassifiedData = knn_func(train,test,k, measure_func, similarity_flag)  # runs knn on each fold

        for dataPoint in splitClassifiedData:
            if "Path" not in dataPoint:  # checks data isnt first data point
                toAdd = [dataPoint[0], dataPoint[1], dataPoint[2], foldNum]  # formats data to add
                output_dataset = numpy.append(output_dataset, [toAdd], axis=0)  # adds data

        results = eval_func(splitClassifiedData)  # gets precision, recall etc

        precisions.append(results[0])  # appends each calculations to a different list
        recalls.append(results[1])
        fMeasures.append(results[2])
        accuracies.append(results[3])

    avg_precision = sum(precisions) / len(precisions)  # calculates the average of the calculations
