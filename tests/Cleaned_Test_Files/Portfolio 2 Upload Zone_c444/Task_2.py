...
def confusionMatrix(classified_data: numpy.typing.NDArray) -> numpy.typing.NDArray:
    data_without_header = classified_data[1:]
    actual_classes = numpy.unique (classified_data[1:, 1])
    predicted_classes = numpy.unique(classified_data[2:, 2])

    confusion_matrix = numpy.zeros((len(actual_classes), len(predicted_classes)), dtype=int)

    classes_order = ['Female', 'Male', 'Primate', 'Rodent', 'Food']
    for i in data_without_header:
        actual = i[1]
        predicted = i[2]

        index_of_actual = 0
        index_of_predicted = 0
        for y, val in enumerate(classes_order):
            if val == actual:
                index_of_actual = y
            if val == predicted:
                index_of_predicted = y
        confusion_matrix[index_of_predicted][index_of_actual] += 1
...
def computeTPs(confusion_matrix: numpy.typing.NDArray) -> list[int]:
    return numpy.diag(confusion_matrix)


def computeFPs(confusion_matrix: numpy.typing.NDArray) -> list[int]:
    return numpy.sum(confusion_matrix, axis=1) - numpy.diag(confusion_matrix)


def computeFNs(confusion_matrix: numpy.typing.NDArray) -> list[int]:
    return numpy.sum(confusion_matrix, axis=0) - numpy.diag(confusion_matrix)
...
def computeAccuracy(tps: list[int], fps: list[int], fns: list[int], data_size: int) -> float:
    return (numpy.sum(tps) + numpy.sum(fns)) / (numpy.sum(tps) + numpy.sum(fps) + numpy.sum(fns) + numpy.sum(fps)) if data_size > 0 else 0.0
