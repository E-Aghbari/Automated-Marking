...
def computeCosineSimilarity(image1: numpy.typing.NDArray, image2: numpy.typing.NDArray) -> float:
    ...
    simg1 = numpy.asarray(image1, dtype=float)
    simg2 = numpy.asarray(image2, dtype=float)
    dot = 0
    s1=0
    s2=0
    for i in range(0,len(simg1)):
        for j in range(0,len(simg1[0])):
            dot +=simg1[i,j]*simg2[i,j]
            s1 += simg1[i,j]*simg1[i,j]
            s2 += simg2[i,j]*simg2[i,j]
