...
def getMostCommonClass(nearest_neighbours_classes: dict[str, int]) -> str:
    ...
    commonClass = []
    i = 0
    for classDict in nearest_neighbours_classes:
        if classDict[i] > highest:
            highest = classDict[i]
            commonClass.append(classDict)
        i += 1

    winner = classification_scheme[commonClass[0]]
...
def kNN(training_data: numpy.typing.NDArray, data_to_classify: numpy.typing.NDArray, k: int, measure_func: Callable,
    ...
    for row in data_to_classify[1:]:
        ...
        # Sort distances/similarities in non-decreasing order
        measures_classes.sort(key=lambda x: x[0])
