...
def getMostCommonClass(nearest_neighbours_classes: dict[str, int]) -> str:
    winner = ''
    if not nearest_neighbours_classes or all(count == 0 for count in nearest_neighbours_classes.values()):
        return winner
    for class_lbl in classification_scheme:
        if class_lbl in nearest_neighbours_classes:
            if nearest_neighbours_classes[class_lbl] > 0:
                winner = class_lbl
                break
    return winner
...
def getClassesOfKNearestNeighbours(measures_classes: list[tuple[float, str]], k: int, similarity_flag: bool) -> (
    ...
    for i in range(stop):
        nearest_neighbours_classes[sorted_measures[i][1]] = nearest_neighbours_classes.get(sorted_measures[i][1],0)+ 1
...
def kNN(training_data: numpy.typing.NDArray, data_to_classify: numpy.typing.NDArray, k: int, measure_func: Callable,
    ...
    classified_data = [data_to_classify[0]]
    read_func
    for data_point in data_to_classify[1:]:
        s_or_d = [measure_func(data_point, train_data_point) for train_data_point in training_data]
        sorted_indicies = sorted(range(len(s_or_d)), key=lambda i: s_or_d[i])
        k_nearers_indicies = sorted_indicies[:k]
        k_nearest_classes = get_neighbour_classes_func([(s_or_d[i], training_data[i][-1]) for i in k_nearers_indicies],
                                                       similarity_flag, k)
        most_common_class = most_common_class_func(k_nearest_classes, classification_scheme)
        classified_data.append(data_point.tolist() + [most_common_class])
