...
def computeBinaryPrecision(tp: int, fp: int) -> float:
    precision = float(0)
    bot = tp + fp
    if bot > 0:
        precision += tp / bot
...
def computeBinaryRecall(tp: int, fn: int) -> float:
    recall = float(0)
    bot = tp + fn
    if bot > 0:
        recall += tp / bot
...
def computeMacroPrecision(tps: list[int], fps: list[int]) -> float:
    precision = float(0)
    num = len(classification_scheme)
    for i in range(0, num):
        precision += computeBinaryPrecision(tps[i], fps[i])
    return precision / num


def computeMacroRecall(tps: list[int], fns: list[int]) -> float:
    num = len(classification_scheme)
    recall = float(0)
    for i in range(0, num):
        recall += computeBinaryRecall(tps[i], fns[i])
    return recall / num


def computeMacroFMeasure(tps: list[int], fps: list[int], fns: list[int]) -> float:
    num = len(classification_scheme)
    f_measure = float(0)
    for i in range(0, num):
...
def computeAccuracy(tps: list[int],data_size: int) -> float:
    if data_size == 0:
        return 0
...
def evaluateKNN(classified_data: numpy.typing.NDArray, confusion_func=confusionMatrix) \
    ...
    precision = computeMacroPrecision(tps, fps)
    recall = computeMacroRecall(tps, fns)
    f_measure = computeMacroFMeasure(tps, fps, fns)
    accuracy = computeAccuracy(tps, data_size)
