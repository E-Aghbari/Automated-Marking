...
def getMostCommonClass(nearest_neighbours_classes: dict[str, int]) -> str:
    if nearest_neighbours_classes:
        return max(nearest_neighbours_classes, key=nearest_neighbours_classes.get)
    else:
        return ''
...
def getClassesOfKNearestNeighbours(measures_classes: list[tuple[float, str]], k: int, similarity_flag: bool) -> (
    ...
    sorted_measures_classes = sorted(measures_classes.items())

    #Slice list to get the first k elements
    nearest_neighbours_classes = {}
    for measure, class_name in sorted_measures_classes[:k]:
...
def kNN(training_data: numpy.typing.NDArray, data_to_classify: numpy.typing.NDArray, k: int, measure_func: Callable,
    ...
    # Have fun!
    for idx, row in enumerate(data_to_classify[1:], start=1):
        image_path = row[0]
        actual_class = row[1]

        # Read and resize image
        image = read_func(image_path)
        print(f"Processing image {idx}/{len(data_to_classify) - 1}...")

        # Calculate distance/similarity to training data
        measures_classes = {}
        for train_row in training_data[1:]:
            train_image_path = train_row[0]
            train_actual_class = train_row[1]
            train_image = read_func(train_image_path)
            measure_value = measure_func(image, train_image)
            measures_classes[measure_value] = train_actual_class

        # Get k nearest neighbours
        neighbour_classes = get_neighbour_classes_func(measures_classes, k, similarity_flag)

        # Get most common class among neighbours
        predicted_class = most_common_class_func(neighbour_classes)
        print(predicted_class)

        # Append result to classified data
