...
def partitionData(training_data: numpy.typing.NDArray, f: int) -> list[numpy.typing.NDArray]:
    ...
    # remove the header
    tr = training_data[1:]
    # This line calculates the size of each fold by performing DIV by f on the length of the training data
    fold_sizes = []
    first_fold_size = math.ceil(len(tr) / f)

    folds_found = False
    loop_count = 1
    while not folds_found:
        # subtract first_fold_size*length of fold_sizes array from length of training data
        calculation = len(tr) - (first_fold_size * (len(fold_sizes) + 1))

        # divide result by f-iteration
        whole_number_hopefully = calculation / (f - loop_count)
        # if whole number, the remaining folds are first_fold_size-1
        if whole_number_hopefully == int(whole_number_hopefully):
            while len(fold_sizes) < f:
                fold_sizes.append(first_fold_size - 1)
                folds_found = True
        # else increase iteration counter and add first_fold_size to fold_sizes array
        else:
            fold_sizes.append(first_fold_size)
            # check if all the fold sizes have been found
            if len(fold_sizes) == f:
                folds_found = True
            else:
                loop_count += 1
    ...
    fold_start = 0
    fold_end = fold_sizes[0]

    for fold in range(f):

        # splice out new testing and training data from fold and add to arrays defined above
        chunk = tr[fold_start:fold_end]

        # add new entry to folds
        folds.append(chunk)

        fold_start = fold_end
        fold_end = fold_start + fold_sizes[fold]
