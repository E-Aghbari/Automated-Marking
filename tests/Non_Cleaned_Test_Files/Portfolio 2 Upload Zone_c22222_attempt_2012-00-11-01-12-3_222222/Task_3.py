...
def partitionData(training_data: numpy.typing.NDArray, f: int) -> list[numpy.typing.NDArray]:
    ...
    for train_index, test_index in kf.split(training_data[1:]):
        X_train_fold, X_test_fold = input_data[train_index], input_data[test_index]
        folds.append((X_train_fold, X_test_fold))  # append a tuple
    return folds
...
def preparingDataForCrossValidation(partition_list: list[numpy.typing.NDArray], f: int) \
    ...
    for i in range(0, f):
        ...
        new_test = partition_list[i][1]
        new_train = partition_list[i][0]
        new_train = numpy.insert(new_train,0, header, axis=0)
        new_test = numpy.insert(new_test, 0, header, axis=0)

        folds.append((numpy.array(new_train), numpy.array(new_test)))
    return folds
...
def crossEvaluateKNN(training_data: numpy.typing.NDArray, f: int, k: int, measure_func: Callable,
    ...
    folds = partition_func(training_data, f)
    folds = prep_func(folds,f)

    precisions = []
    recalls = []
    f_measures = []
    accuracies = []
    classified_data_list = []  # Initialize an empty list

    # Perform cross-validation
    for i, (train_index, test_index) in enumerate(folds):

        # Ensure train_index and test_index are of integer type
        train_index = numpy.array(train_index).astype(int)
        test_index = numpy.array(test_index).astype(int)

        train_data, test_data = training_data[train_index], training_data[test_index]

        # Classify the data
        classified_data = knn_func(train_data,  k, test_data, measure_func, similarity_flag)
        classified_data["FoldNumber"] = i

        # Evaluate the classified data
        precision, recall, f_measure, accuracy = eval_func([classified_data])
        precisions.append(precision)
        recalls.append(recall)
        f_measures.append(f_measure)
        accuracies.append(accuracy)

        # Append the classified data to the list
        classified_data_list.append(classified_data)

    # Calculate the average precision, recall, F-measure, and accuracy
    ...
    processed = numpy.concatenate(classified_data_list)
    processed = numpy.append(processed, [["avg_precision", "avg_recall", "avg_f_measure", "avg_accuracy"],
                                         [avg_precision, avg_recall, avg_f_measure, avg_accuracy]], axis=0)
